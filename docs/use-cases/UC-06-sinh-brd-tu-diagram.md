...
- Provider credential (OpenAI hoặc tương đương) đã cấu hình ở backend; **frontend không cầm key**.
...
- Sticky note không tạo step mới mà chỉ ảnh hưởng `assumptions` / `annotations` / `open_questions`.
...
- BRD output có section `Loops` mô tả các node tham gia vòng lặp và lý do (note đính kèm nếu có).
...
- BRD draft đưa nội dung note vào section `Annotations` cuối tài liệu, không gắn vào bất kỳ step cụ thể nào.
